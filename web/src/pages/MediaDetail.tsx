...
export default function MediaDetail() {
  ...
  useEffect(() => {
    ...
    axios
      .get("/api/lists/me", { headers: { Authorization: `Bearer ${token}` } })
      .then((res) => {
        setLists(res.data);
            ...
            {directors.map((member) => (
              <div key={member.id} className="cast-card">
                ...
                <p className="cast-role">
                  {media.media_type === "movie" ? "Réalisateur" : "Créateur"}
                </p>
              </div>
            ))}
          </div>
            ...
            {showCreateInput && (
              <div className="list-create-inline">
                <input
                  ...
                  autoFocus
                />
                <button onClick={createAndAdd}>Créer</button>
              </div>
            )}
          </div>
        ) : (
          <p className="guest-msg">
            <Link to="/login">Connectez-vous</Link> pour noter, commenter et
            enregistrer cette œuvre.
          </p>
        )}
